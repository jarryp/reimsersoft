...
echo "<form method='post' action='php/lista_depositos2.php' target='contenido1'";
echo "<center>";
echo "	<table>";
echo "<tr><th colspan='5'> Depositos Rango de Fechas </th></tr>";
...
$conn=conectarpg();
if($conn){
  $query="select nom_ban from banco order by nom_ban";
  $result=pg_query($query);
  if($result){
    echo "<select id='cmb_banco' name='cmb_banco'>";
    while ($row=pg_fetch_assoc($result)) {
      echo "<option value='". $row['nom_ban'] ."'>". $row['nom_ban'] ."</option>";
    }
    echo "</select>";
  }
}else{
